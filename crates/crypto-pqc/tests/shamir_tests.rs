...
#[test]
fn test_shamir_split_combine_2_of_3() {
    let secret = [42u8; 32];
...
#[test]
fn test_shamir_split_combine_3_of_5() {
...
#[test]
fn test_shamir_split_combine_4_of_7() {
    let secret = [0x01u8; 32];
    let shares = ShamirSecretSharing::split(&secret, 7, 4).unwrap();
    assert_eq!(shares.len(), 7);

    let recovered = ShamirSecretSharing::combine(&shares[0..4], 4).unwrap();
    assert_eq!(recovered, secret);
}

#[test]
fn test_shamir_split_combine_5_of_10() {
    let secret = [0xFFu8; 32];
    let shares = ShamirSecretSharing::split(&secret, 10, 5).unwrap();
    assert_eq!(shares.len(), 10);

    let recovered = ShamirSecretSharing::combine(&shares[0..5], 5).unwrap();
    assert_eq!(recovered, secret);
}

#[test]
fn test_shamir_maximum_threshold() {
    let secret = [77u8; 32];
    let shares = ShamirSecretSharing::split(&secret, 3, 3).unwrap();
    assert_eq!(shares.len(), 3);

    let recovered = ShamirSecretSharing::combine(&shares, 3).unwrap();
    assert_eq!(recovered, secret);
}

#[test]
fn test_shamir_not_enough_shares_fails() {
...
#[test]
fn test_shamir_exact_threshold_recovers_secret() {
...
#[test]
fn test_shamir_different_secrets_produce_different_shares() {
...
#[test]
fn test_shamir_any_subset_of_threshold_works() {
    let secret = [0x99u8; 32];
    let shares = ShamirSecretSharing::split(&secret, 5, 3).unwrap();

    let recovered_1 = ShamirSecretSharing::combine(&[shares[0].clone(), shares[2].clone(), shares[4].clone()], 3).unwrap();
    assert_eq!(recovered_1, secret);

    let recovered_2 = ShamirSecretSharing::combine(&[shares[1].clone(), shares[3].clone(), shares[4].clone()], 3).unwrap();
...
#[test]
fn test_shamir_secret_with_ascii_pattern() {
